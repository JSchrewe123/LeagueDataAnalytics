...
public class Summoner {
    ...
    public Summoner(String name, String reg, String apiName) throws NotValidRegionException, NotValidNameException {
        ...
        if (this.reg.equals("euw1") || this.reg.equals("eun1") || this.reg.equals("ru") || this.reg.equals("tr1")) {
            this.bigReg = "europe";
        } else if (this.reg.equals("na1") || this.reg.equals("la1") || this.reg.equals("la2") || this.reg.equals("br1")
                || this.reg.equals("oc1")) {
            this.bigReg = "americas";
        } else if (this.reg.equals("jp1") || this.reg.equals("kr")) {
            this.bigReg = "europe";
        } else {
            throw new NotValidRegionException("Region does not exist: " + reg);
        }
        ...
        try {
            JSONObject json = JsonReader.readJsonFromUrl("https://" + this.reg
                    + ".api.riotgames.com/lol/summoner/v4/summoners/by-name/" + this.apiName + "?api_key=" + key);
        ...
        if (rankedData[3] != null) {
            ...
            this.soloWr = "" + Math.round(Double.parseDouble(soloWins)
                    / (Double.parseDouble(soloWins) + Double.parseDouble(soloLosses)) * 100);
        } else {
            this.soloLosses = "0";
            this.soloWr = "0";
        }
        ...
        if (rankedData[7] != null) {
            ...
            this.flexWr = "" + Math.round(Double.parseDouble(flexWins)
                    / (Double.parseDouble(flexWins) + Double.parseDouble(flexLosses)) * 100);
        } else {
            this.flexLosses = "0";
            this.flexWr = "0";
        }
    ...
    public String getSummonerData(String data) {
        JSONObject json;
        try {
            json = JsonReader.readJsonFromUrl("https://" + reg + ".api.riotgames.com/lol/summoner/v4/summoners/by-name/"
                    + apiName + "?api_key=" + key);
    ...
    public String[] getRankedData(String id) throws NotValidRegionException {
        try {
            URL url = new URL("https://" + reg + ".api.riotgames.com/lol/league/v4/entries/by-summoner/" + id
                    + "?api_key=" + key);
            ...
            for (int i = 0; i < json_arr.length(); i++) { // OR iterate
                ...
                if (tmp.get("queueType").equals("RANKED_SOLO_5x5")) {
                    try {
                        data[0] = (String) tmp.get("tier") + " " + (String) tmp.get("rank") + " "
                                + tmp.get("leaguePoints") + "LP";
                        data[2] = "" + tmp.get("wins");
                    ...
                    try {
                        data[4] = (String) tmp.get("tier") + " " + (String) tmp.get("rank") + " "
                                + tmp.get("leaguePoints") + "LP";
                        data[6] = "" + tmp.get("wins");
    ...
    public String toString() {
        try {
            ...
            try {
                soloqWr = Math.round(
                        Double.parseDouble(data[2]) / (Integer.parseInt(data[2]) + Integer.parseInt(data[3])) * 100)
                        + "%wr";
                if (data[1] != null) {
            ...
            try {
                flexqWr = Math.round(
                        Double.parseDouble(data[6]) / (Integer.parseInt(data[6]) + Integer.parseInt(data[7])) * 100)
                        + "%wr";
                if (data[5] != null) {
            ...
            return name + " , lvl " + getIntSummonerData("summonerLevel") + " , SoloQ: " + data[0] + " " + soloqWr
                    + ", FlexQ: " + data[4] + " " + flexqWr;
        } catch (Exception e) {
